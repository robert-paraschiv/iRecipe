...
public class profileEditProfile extends Fragment {
    ...
    private void updateLikes() {
        //Update user likes with name and profile pic
        db.collectionGroup("UsersWhoFaved").whereEqualTo("userID", mUser.getUser_id()).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(final QuerySnapshot queryDocumentSnapshots) {
                if (queryDocumentSnapshots != null) {
                    if (queryDocumentSnapshots.size() > 0) {
                        likesUpdated = 0;
                        Log.d(TAG, "onSuccess: UsersWhoFaved " + queryDocumentSnapshots.size());
                        for (QueryDocumentSnapshot documentSnapshot : queryDocumentSnapshots) {
                            UserWhoFaved userWhoFaved = documentSnapshot.toObject(UserWhoFaved.class);
                            if (userWhoFaved.getUser_name() == null)
                                userWhoFaved.setUser_name(mUser.getName());
                            else if (!userWhoFaved.getUser_name().equals(mUser.getName()))
                                userWhoFaved.setUser_name(mUser.getName());
                            if (userWhoFaved.getUser_imageUrl() == null)
                                userWhoFaved.setUser_imageUrl(mUser.getUserProfilePicUrl());
                            else if (!userWhoFaved.getUser_imageUrl().equals(mUser.getUserProfilePicUrl()))
                                userWhoFaved.setUser_imageUrl(mUser.getUserProfilePicUrl());

                            final DocumentReference documentReference = documentSnapshot.getReference();
                            documentReference.set(userWhoFaved).addOnSuccessListener(new OnSuccessListener<Void>() {
                                @Override
                                public void onSuccess(Void aVoid) {
                                    Log.d(TAG, "onSuccess: Updated " + documentReference.getPath());
                                    likesUpdated++;
                                    if (likesUpdated == queryDocumentSnapshots.size())
                                        updateENDED();
                                }
                            });
                        }
    ...
    private void updateRecipes() {
        //Update recipes of user with new name and profile pic
        db.collection("Recipes").whereEqualTo("creator_docId", mUser.getUser_id()).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(final QuerySnapshot queryDocumentSnapshots) {
                if (queryDocumentSnapshots != null) {
                    if (queryDocumentSnapshots.size() > 0) {
                        recipesUpdated = 0;
                        Log.d(TAG, "onSuccess: Recipes " + queryDocumentSnapshots.size());
                        for (QueryDocumentSnapshot documentSnapshot : queryDocumentSnapshots) {
                            Recipe recipe = documentSnapshot.toObject(Recipe.class);
                            if (recipe.getCreator_name() == null)
                                recipe.setCreator_name(mUser.getName());
                            else if (!recipe.getCreator_name().equals(mUser.getName()))
                                recipe.setCreator_name(mUser.getName());
                            if (recipe.getCreator_imageUrl() == null)
                                recipe.setCreator_imageUrl(mUser.getUserProfilePicUrl());
                            else if (!recipe.getCreator_imageUrl().equals(mUser.getUserProfilePicUrl()))
                                recipe.setCreator_imageUrl(mUser.getUserProfilePicUrl());
                            ...
                            documentReference.set(recipe).addOnSuccessListener(new OnSuccessListener<Void>() {
                                ...
                                public void onSuccess(Void aVoid) {
                                    Log.d(TAG, "onSuccess: Updated " + documentReference.getPath());
                                    recipesUpdated++;
                                    if (recipesUpdated == queryDocumentSnapshots.size())
                                        updateLikes();
                                }
                            });
                        }
    ...
    private void updatePosts() {
        //Update user posts with new name and profile pic
        db.collection("Posts").whereEqualTo("creatorId", mUser.getUser_id()).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(final QuerySnapshot queryDocumentSnapshots) {
                if (queryDocumentSnapshots != null) {
                    if (queryDocumentSnapshots.size() > 0) {
                        postsUpdated = 0;
                        Log.d(TAG, "onSuccess: Posts " + queryDocumentSnapshots.size());
                        for (QueryDocumentSnapshot documentSnapshot : queryDocumentSnapshots) {
                            Post post = documentSnapshot.toObject(Post.class);
                            if (post.getCreator_name() == null)
                                post.setCreator_name(mUser.getName());
                            else if (!post.getCreator_name().equals(mUser.getName()))
                                post.setCreator_name(mUser.getName());
                            if (post.getCreator_imageUrl() == null)
                                post.setCreator_imageUrl(mUser.getUserProfilePicUrl());
                            else if (!post.getCreator_imageUrl().equals(mUser.getUserProfilePicUrl()))
                                post.setCreator_imageUrl(mUser.getUserProfilePicUrl());
                            ...
                            documentReference.set(post).addOnSuccessListener(new OnSuccessListener<Void>() {
                                ...
                                public void onSuccess(Void aVoid) {
                                    Log.d(TAG, "onSuccess: Updated " + documentReference.getPath());
                                    postsUpdated++;
                                    if (postsUpdated == queryDocumentSnapshots.size())
                                        updateRecipes();
                                }
                            });
                        }
    ...
    private void updateComments() {
        //Update user comments with new name and profile pic
        db.collectionGroup("Comments").whereEqualTo("user_id", mUser.getUser_id()).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(final QuerySnapshot queryDocumentSnapshots) {
                if (queryDocumentSnapshots != null) {
                    if (queryDocumentSnapshots.size() > 0) {
                        commentsUpdated = 0;
                        Log.d(TAG, "onSuccess: Comments " + queryDocumentSnapshots.size());
                        for (QueryDocumentSnapshot documentSnapshot : queryDocumentSnapshots) {
                            Comment comment = documentSnapshot.toObject(Comment.class);
                            if (comment.getUser_name() == null)
                                comment.setUser_name(mUser.getName());
                            else if (!comment.getUser_name().equals(mUser.getName()))
                                comment.setUser_name(mUser.getName());
                            if (comment.getUser_profilePic() == null)
                                comment.setUser_profilePic(mUser.getUserProfilePicUrl());
                            else if (!comment.getUser_profilePic().equals(mUser.getUserProfilePicUrl()))
                                comment.setUser_profilePic(mUser.getUserProfilePicUrl());
                            ...
                            documentReference.set(comment).addOnSuccessListener(new OnSuccessListener<Void>() {
                                ...
                                public void onSuccess(Void aVoid) {
                                    Log.d(TAG, "onSuccess: Updated " + documentReference.getPath());
                                    commentsUpdated++;
                                    if (commentsUpdated == queryDocumentSnapshots.size())
                                        updatePosts();
                                }
                            });
                        }
    ...
    private void updateFriendList() {
        //Update FriendLists of every user who has current user as friend with new user name and profile pic
        db.collectionGroup("FriendList").whereEqualTo("friend_user_id", mUser.getUser_id()).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(final QuerySnapshot queryDocumentSnapshots) {
                if (queryDocumentSnapshots != null) {
                    if (queryDocumentSnapshots.size() > 0) {
                        friendsUpdated = 0;
                        Log.d(TAG, "onSuccess: FriendList " + queryDocumentSnapshots.size());
                        for (QueryDocumentSnapshot documentSnapshot : queryDocumentSnapshots) {
                            Friend friend = documentSnapshot.toObject(Friend.class);
                            if (friend.getFriend_user_name() == null)
                                friend.setFriend_user_name(mUser.getName());
                            else if (!friend.getFriend_user_name().equals(mUser.getName()))
                                friend.setFriend_user_name(mUser.getName());

                            if (friend.getFriend_user_profilePic() == null)
                                friend.setFriend_user_profilePic(mUser.getUserProfilePicUrl());
                            else if (!friend.getFriend_user_profilePic().equals(mUser.getUserProfilePicUrl()))
                                friend.setFriend_user_profilePic(mUser.getUserProfilePicUrl());

                            final DocumentReference documentReference = documentSnapshot.getReference();
                            documentReference.set(friend).addOnSuccessListener(new OnSuccessListener<Void>() {
                                @Override
                                public void onSuccess(Void aVoid) {
                                    Log.d(TAG, "onSuccess: Updated " + documentReference.getPath());
                                    friendsUpdated++;
                                    if (friendsUpdated == queryDocumentSnapshots.size())
                                        updateComments();
                                }
                            });
                        }
    ...
    private void updateConversations() {
        //Update every conversations of user with new name and profile pic of user
        db.collectionGroup("Conversations").whereEqualTo("userId", mUser.getUser_id()).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(final QuerySnapshot queryDocumentSnapshots) {
                if (queryDocumentSnapshots != null) {
                    if (queryDocumentSnapshots.size() > 0) {
                        Log.d(TAG, "onSuccess: Conversations " + queryDocumentSnapshots.size());
                        conversationsUpdated = 0;
                        for (QueryDocumentSnapshot documentSnapshot : queryDocumentSnapshots) {
                            Conversation conversation = documentSnapshot.toObject(Conversation.class);
                            if (conversation.getUser_name() == null)
                                conversation.setUser_name(mUser.getName());
                            if (!conversation.getUser_name().equals(mUser.getName())) {
                                conversation.setUser_name(mUser.getName());
                            }
                            if (conversation.getUser_profilePic() != null && !conversation.getUser_profilePic().equals(mUser.getUserProfilePicUrl())) {
                                conversation.setUser_profilePic(mUser.getUserProfilePicUrl());
                            } else if (conversation.getUser_profilePic() == null) {
                                conversation.setUser_profilePic(mUser.getUserProfilePicUrl());
                            }
                            final DocumentReference documentReference = documentSnapshot.getReference();
                            documentReference.set(conversation).addOnSuccessListener(new OnSuccessListener<Void>() {
                                ...
                                public void onSuccess(Void aVoid) {
                                    ...
                                    if (conversationsUpdated == queryDocumentSnapshots.size())
                                        updateFriendList();
                                }
                            });
                        }
